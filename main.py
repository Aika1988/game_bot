...
#     bot.send_sticker(message.chat.id,'CAACAgIAAxkBAAJKgWOhPi0GVzYQO5GKu26m4MxWCvH-AAL7DAAC0z9hS-vkFkvmPoUiLAQ')
#     bot.send_photo(message.chat.id,'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Python-logo-notext.svg/1200px-Python-logo-notext.svg.png' )
...
def answer_check(msg):
    if msg.text == 'Да':
        ...
        p = 3
        start_game(msg, random_number, p-1)

    else:
        bot.send_message(msg.chat.id, 'Ну и ладно!')

def start_game(msg, random_number, p):
    msg = bot.send_message(msg.chat.id, 'Ввведи число от 1 до 10: ')  
...
def check_func(msg, random_number, p):
    if msg.text == str(random_number):
        ...
        bot.send_message(msg.chat.id, f'Попробуй еще раз, у тебя осталась {p} попыток')
